...
const { generateRandomLetter } = require('./util');
...
const YesIntent = {
  ...
  handle(handlerInput) {
    ...
    sessionAttributes.gameState = 'STARTED';
    [sessionAttributes.alphabetPosition, sessionAttributes.alphabetLetter] = generateRandomLetter();

    return handlerInput.responseBuilder
      .speak(requestAttributes.t('YES_MESSAGE', sessionAttributes.alphabetLetter))
      .reprompt(requestAttributes.t('HELP_REPROMPT'))
      .getResponse();
  },
};
...
const NumberGuessIntent = {
  ...
  async handle(handlerInput) {
    ...
    const targetNum = parseInt(sessionAttributes.alphabetPosition) + 1;
